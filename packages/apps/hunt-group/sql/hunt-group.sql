...
CREATE TABLE IF NOT EXISTS hunt_group_calls
(
    id            SERIAL PRIMARY KEY NOT NULL,
    hunt_group_id INTEGER            NOT NULL,
    call_sid      VARCHAR(50)        NOT NULL,
    parent_sid    VARCHAR(50)        NOT NULL,
    callTime      TIMESTAMPTZ        NOT NULL,
    CONSTRAINT fk_hunt_group_id
        FOREIGN KEY (hunt_group_id)
            REFERENCES hunt_groups (id)
);

CREATE UNIQUE INDEX IF NOT EXISTS hunt_group_call_sid_idx ON hunt_group_calls (call_sid);